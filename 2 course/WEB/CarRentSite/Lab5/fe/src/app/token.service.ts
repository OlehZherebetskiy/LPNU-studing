...
export class TokenService {
  ...
  refreshTokenSubs(): any {
    return new Promise((resolve, reject) => {
      this.refreshToken().subscribe(
        this.refreshTokenSubData(resolve),
        this.refreshTokenSubError(reject)
      );
    });
  }

  refreshTokenSubData(resolve): any {
    return data => {
  ...
  refreshTokenSubError(reject): any {
    return error => {
      console.log(error)
      reject(error);
    }
  ...
  verifyTokenSubs(): any {
    return new Promise((resolve, reject) => {
      this.verifyToken().subscribe(
        this.verifyTokenSubData(resolve),
        this.verifyTokenSubError(reject)
      );
    });
  }

  verifyTokenSubData(resolve): any {
    return data => {
      resolve();
    }
  }

  verifyTokenSubError(reject): any {
    return error => {
      console.log(error);
      reject(error)
    }
